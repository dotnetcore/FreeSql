...
{
    class TDengineAdo : FreeSql.Internal.CommonProvider.AdoProvider
    {
        ...
        public TDengineAdo(CommonUtils util, string masterConnectionString, string[] slaveConnectionStrings, Func<DbConnection> connectionFactory) : base(DataType.TDengine, masterConnectionString, slaveConnectionStrings)
        {
            ...
            {
                var pool = new FreeSql.Internal.CommonProvider.DbConnectionPool(DataType.TDengine, connectionFactory);
                MasterPool = pool;
                _CreateCommandConnection = pool.TestConnection;
                return;
            }
            if (!string.IsNullOrEmpty(masterConnectionString))
                MasterPool = new TDengineConnectionPool("主库", masterConnectionString, null, null);
            if (slaveConnectionStrings != null)
            {
                foreach (var slaveConnectionString in slaveConnectionStrings)
                {
                    var slavePool = new TDengineConnectionPool($"从库{SlavePools.Count + 1}", slaveConnectionString, () => Interlocked.Decrement(ref slaveUnavailables), () => Interlocked.Increment(ref slaveUnavailables));
        ...
        {
            var rawPool = pool as TDengineConnectionPool;
