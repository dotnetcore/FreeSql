...
{
    public class LazyLoadingNotFundTest
    {

        [Table(Name = "pgarray_tomany_user_lazyloading_notfound")]
        public class User
        {
        ...
        [Table(Name = "pgarray_tomany_role_lazyloading_notfound")]
        public class Role
        {
            [Column(IsPrimary = true)]
        ...
        public void LazyLoading()
        {
            ...
            var users = new[]
            {
                new User { UserId = 11, RoleIds = new [] { 1,5,2 }, UserName = "user1" },
                new User { UserId = 12, RoleIds = new [] { 1,2,5,3 }, UserName = "user2" },
                new User { UserId = 13, RoleIds = new [] { 1,5,3 }, UserName = "user3" },
                new User { UserId = 14, RoleIds = new [] { 3,2,5,1 }, UserName = "user4" },
...
WHERE (exists(SELECT 1 
    ...
    limit 1))", roles5Select.ToSql());
            ...
            Assert.Equal("1,5,2", string.Join(",", users2[0].RoleIds));
            Assert.Equal("1,2,5,3", string.Join(",", users2[1].RoleIds));
            Assert.Equal("1,5,3", string.Join(",", users2[2].RoleIds));
            Assert.Equal("3,2,5,1", string.Join(",", users2[3].RoleIds));
            ...
            Assert.Null(roles2[1]);
            Assert.Equal(2, roles2[2].RoleId);
            Assert.Equal("role2", roles2[2].RoleName);
            ...
            Assert.Null(roles2[2]);
            Assert.Equal(3, roles2[3].RoleId);
            Assert.Equal("role3", roles2[3].RoleName);
            ...
            Assert.Equal("role2", roles2[1].RoleName);
            Assert.Null(roles2[2]);
            Assert.Equal(1, roles2[3].RoleId);
            Assert.Equal("role1", roles2[3].RoleName);
