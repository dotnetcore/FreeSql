...
namespace FreeSql.Tests.GBase
{
    public class GBaseSelectTest
    {
        ...
        {
            //OneToOne、ManyToOne
            var t0 = g.gbase.Select<Tag>().Where(a => a.Parent.Parent.Name == "粤语").ToSql();
...
WHERE (exists(SELECT FIRST 1 1 
    ...
    WHERE (Mt_Ms.Song_id = a.Id) AND (exists(SELECT FIRST 1 1 
        ...
        public void Lazy()
        {
            var tags = g.gbase.Select<Tag>().Where(a => a.Parent.Name == "xxx")
                .LeftJoin(a => a.Parent_id == a.Parent.Id)
                .ToSql();

            var songs = g.gbase.Select<Song>().Limit(10).ToList();
        }
        ...
        public void ToDataTable()
        {
            ...
            //Assert.Equal(1, g.firebird.Insert<TopicInserts>().AppendData(items.First()).ExecuteAffrows());
            Assert.Equal(11, g.gbase.Insert<TopicInserts>().AppendData(items).ExecuteAffrows());

            //items = Enumerable.Range(0, 9989).Select(a => new TopicInserts { Title = "newtitle" + a, CreateTime = DateTime.Now }).ToList();
            //Assert.Equal(9989, g.firebird.Insert<TopicInserts>(items).ExecuteAffrows());
        ...
        public void ToList()
        {
            ...
            g.gbase.Insert<TestGuidIdToList>().AppendData(new TestGuidIdToList()).ExecuteAffrows();
            var testGuidId5 = g.gbase.Select<TestGuidIdToList>().ToList();
            var testGuidId6 = g.gbase.Select<TestGuidIdToList>().ToList(a => a.id);
            ...
            var ddd = g.gbase.Select<District>().LeftJoin(d => d.ParentCode == d.Parent.Code).ToTreeList();
            Assert.Single(ddd);
        ...
        public void GroupBy()
        {
            ...
            var testpid1 = g.gbase.Insert<TestTypeInfo>().AppendData(new TestTypeInfo { Name = "Name" + DateTime.Now.ToString("yyyyMMddHHmmss") }).ExecuteIdentity();
            g.gbase.Insert<TestInfo>().AppendData(new TestInfo { Title = "Title" + DateTime.Now.ToString("yyyyMMddHHmmss"), CreateTime = DateTime.Now, TypeGuid = (int)testpid1 }).ExecuteAffrows();
...
FROM tb_topic22 a 
WHERE (((cast(a.Id as varchar(8000))) in (SELECT b.Title 
    FROM tb_topic22 b)))", subquery);
            ...
            Assert.Equal(@"SELECT a.Id, a.Clicks, a.TypeGuid, a.Title, a.CreateTime 
FROM tb_topic22 a 
WHERE (((cast(a.Id as varchar(8000))) in ( SELECT * FROM (SELECT FIRST 10 b.Title 
    FROM tb_topic22 b) ftblmt50 )))", subquery);
        ...
        public void AsTable()
        {
            ...
            sql = query.ToSql().Replace("\r\n", "");
            Assert.Equal("SELECT a.Id, a.Clicks, a.TypeGuid, a.Title, a.CreateTime FROM tb_topic22_T1 a LEFT JOIN TestTypeInfo b on b.Guid = a.TypeGuid and b.Name = ?", sql);
        ...
        public void Include_OneToMany2()
        {
            ...
            model2.id = (int)g.gbase.Insert(model2).ExecuteIdentity();
            ...
            var at1 = g.gbase.Select<TiOtmModel11>()
                .LeftJoin(a => a.model2id == a.model2.id)
            ...
            var at11 = g.gbase.Select<TiOtmModel11>()
                .LeftJoin(a => a.model2id == a.model2.id)
        ...
        public void Include_OneToChilds()
        {
            ...
            tag1.Id = (int)g.gbase.Insert(tag1).ExecuteIdentity();
            var tag1_1 = new Tag
            {
                Parent_id = tag1.Id,
                Ddd = DateTime.Now.Second,
            ...
            tag1_1.Id = (int)g.gbase.Insert(tag1_1).ExecuteIdentity();
            var tag1_2 = new Tag
            {
                Parent_id = tag1.Id,
                Ddd = DateTime.Now.Second,
            ...
            tag1_2.Id = (int)g.gbase.Insert(tag1_2).ExecuteIdentity();
            ...
            tag2.Id = (int)g.gbase.Insert(tag2).ExecuteIdentity();
            var tag2_1 = new Tag
            {
                Parent_id = tag2.Id,
                Ddd = DateTime.Now.Second,
            ...
            tag2_1.Id = (int)g.gbase.Insert(tag2_1).ExecuteIdentity();
            var tag2_2 = new Tag
            {
                Parent_id = tag2.Id,
                Ddd = DateTime.Now.Second,
            ...
            tag2_2.Id = (int)g.gbase.Insert(tag2_2).ExecuteIdentity();

            var tags0 = g.gbase.Select<Tag>()
                .Include(a => a.Parent)
                .Where(a => a.Id == tag1.Id || a.Id == tag2.Id)
                .ToList();

            var tags1 = g.gbase.Select<Tag>()
            ...
            var atags0 = g.gbase.Select<Tag>()
                .Include(a => a.Parent)
                .Where(a => a.Id == tag1.Id || a.Id == tag2.Id)
                .ToList();

            var atags1 = g.gbase.Select<Tag>()
            ...
            var atags11 = g.gbase.Select<Tag>()
            ...
            var atags22 = g.gbase.Select<Tag>()
                .IncludeMany(a => a.Tags.Take(1).Select(b => new Tag { Id = b.Id, Name = b.Name }),
            ...
            var atags33 = g.gbase.Select<Tag>()
                .IncludeMany(a => a.Tags.Take(1).Select(b => new Tag { Id = b.Id, Name = b.Name }),
        ...
        public void Include_ManyToMany()
        {
            ...
            tag3.Id = (int)g.gbase.Insert(tag3).ExecuteIdentity();
            ...
            song1.Id = (int)g.gbase.Insert(song1).ExecuteIdentity();
            var song2 = new Song
            {
                Create_time = DateTime.Now,
            ...
            song2.Id = (int)g.gbase.Insert(song2).ExecuteIdentity();
            var song3 = new Song
            {
                Create_time = DateTime.Now,
            ...
            song3.Id = (int)g.gbase.Insert(song3).ExecuteIdentity();

            g.gbase.Insert(new Song_tag { Song_id = song1.Id, Tag_id = tag1.Id }).ExecuteAffrows();
            g.gbase.Insert(new Song_tag { Song_id = song2.Id, Tag_id = tag1.Id }).ExecuteAffrows();
            g.gbase.Insert(new Song_tag { Song_id = song3.Id, Tag_id = tag1.Id }).ExecuteAffrows();
            g.gbase.Insert(new Song_tag { Song_id = song1.Id, Tag_id = tag2.Id }).ExecuteAffrows();
            g.gbase.Insert(new Song_tag { Song_id = song3.Id, Tag_id = tag2.Id }).ExecuteAffrows();
            g.gbase.Insert(new Song_tag { Song_id = song3.Id, Tag_id = tag3.Id }).ExecuteAffrows();

            var songs1 = g.gbase.Select<Song>()
                .IncludeMany(a => a.Tags)
            ...
            var songs2 = g.gbase.Select<Song>()
                .IncludeMany(a => a.Tags,
            ...
            var tags3 = g.gbase.Select<Song_tag>()
            ...
            var songs11 = g.gbase.Select<Song>()
                .IncludeMany(a => a.Tags.Take(1))
                .Where(a => a.Id == song1.Id || a.Id == song2.Id || a.Id == song3.Id)
                .ToList();
            ...
            var songs22 = g.gbase.Select<Song>()
                .IncludeMany(a => a.Tags.Take(1),
            ...
            var tags33 = g.gbase.Select<Song_tag>()
                .Include(a => a.Tag.Parent)
            ...
            new List<Song>(new[] { song1, song2, song3 }).IncludeMany(g.gbase, a => a.Tags.Select(b => new Tag { Id = b.Id, Name = b.Name }));

            var asongs1 = g.gbase.Select<Song>()
                .IncludeMany(a => a.Tags.Select(b => new Tag { Id = b.Id, Name = b.Name }))
                .Where(a => a.Id == song1.Id || a.Id == song2.Id || a.Id == song3.Id)
                .ToList();
            Assert.Equal(3, songs1.Count);
            ...
            var asongs2 = g.gbase.Select<Song>()
                .IncludeMany(a => a.Tags.Select(b => new Tag { Id = b.Id, Name = b.Name }),
            ...
            var atags3 = g.gbase.Select<Song_tag>()
                .Include(a => a.Tag.Parent)
            ...
            var asongs11 = g.gbase.Select<Song>()
                .IncludeMany(a => a.Tags.Take(1).Select(b => new Tag { Id = b.Id, Name = b.Name }))
                .Where(a => a.Id == song1.Id || a.Id == song2.Id || a.Id == song3.Id)
                .ToList();
            Assert.Equal(3, songs11.Count);
            ...
            var asongs22 = g.gbase.Select<Song>()
                .IncludeMany(a => a.Tags.Take(1).Select(b => new Tag { Id = b.Id, Name = b.Name }),
            ...
            var atags33 = g.gbase.Select<Song_tag>()
                .Include(a => a.Tag.Parent)
        ...
        {
            g.gbase.Select<ToDel1Pk>().ToDelete().ExecuteAffrows();
            Assert.Equal(0, g.gbase.Select<ToDel1Pk>().Count());
            g.gbase.Insert(new[] {
            ...
            Assert.Equal(2, g.gbase.Select<ToDel1Pk>().Where(a => a.name.StartsWith("name")).ToDelete().ExecuteAffrows());
            Assert.Equal(3, g.gbase.Select<ToDel1Pk>().Count());
            Assert.Equal(3, g.gbase.Select<ToDel1Pk>().Where(a => a.name.StartsWith("nick")).Count());

            g.gbase.Select<ToDel2Pk>().ToDelete().ExecuteAffrows();
            Assert.Equal(0, g.gbase.Select<ToDel2Pk>().Count());
            g.gbase.Insert(new[] {
            ...
            Assert.Equal(2, g.gbase.Select<ToDel2Pk>().Where(a => a.name.StartsWith("name")).ToDelete().ExecuteAffrows());
            Assert.Equal(3, g.gbase.Select<ToDel2Pk>().Count());
            Assert.Equal(3, g.gbase.Select<ToDel2Pk>().Where(a => a.name.StartsWith("nick")).Count());

            g.gbase.Select<ToDel3Pk>().ToDelete().ExecuteAffrows();
            Assert.Equal(0, g.gbase.Select<ToDel3Pk>().Count());
            g.gbase.Insert(new[] {
            ...
            Assert.Equal(2, g.gbase.Select<ToDel3Pk>().Where(a => a.name.StartsWith("name")).ToDelete().ExecuteAffrows());
            Assert.Equal(3, g.gbase.Select<ToDel3Pk>().Count());
            Assert.Equal(3, g.gbase.Select<ToDel3Pk>().Where(a => a.name.StartsWith("nick")).Count());
        }
        ...
        {
            g.gbase.Select<ToUpd1Pk>().ToDelete().ExecuteAffrows();
            Assert.Equal(0, g.gbase.Select<ToUpd1Pk>().Count());
            g.gbase.Insert(new[] {
            ...
            Assert.Equal(2, g.gbase.Select<ToUpd1Pk>().Where(a => a.name.StartsWith("name")).ToUpdate().Set(a => a.name, "nick?").ExecuteAffrows());
            Assert.Equal(5, g.gbase.Select<ToUpd1Pk>().Count());
            Assert.Equal(5, g.gbase.Select<ToUpd1Pk>().Where(a => a.name.StartsWith("nick")).Count());

            g.gbase.Select<ToUpd2Pk>().ToDelete().ExecuteAffrows();
            Assert.Equal(0, g.gbase.Select<ToUpd2Pk>().Count());
            g.gbase.Insert(new[] {
            ...
            Assert.Equal(2, g.gbase.Select<ToUpd2Pk>().Where(a => a.name.StartsWith("name")).ToUpdate().Set(a => a.name, "nick?").ExecuteAffrows());
            Assert.Equal(5, g.gbase.Select<ToUpd2Pk>().Count());
            Assert.Equal(5, g.gbase.Select<ToUpd2Pk>().Where(a => a.name.StartsWith("nick")).Count());

            g.gbase.Select<ToUpd3Pk>().ToDelete().ExecuteAffrows();
            Assert.Equal(0, g.gbase.Select<ToUpd3Pk>().Count());
            g.gbase.Insert(new[] {
            ...
            Assert.Equal(2, g.gbase.Select<ToUpd3Pk>().Where(a => a.name.StartsWith("name")).ToUpdate().Set(a => a.name, "nick?").ExecuteAffrows());
            Assert.Equal(5, g.gbase.Select<ToUpd3Pk>().Count());
            Assert.Equal(5, g.gbase.Select<ToUpd3Pk>().Where(a => a.name.StartsWith("nick")).Count());
        }
        ...
        public void ForUpdate()
        {
            ...
            orm.Transaction(() =>
            {
                ...
                Assert.Equal("SELECT FIRST 1 a.id, a.name FROM ToUpd1Pk a for update", sql);
